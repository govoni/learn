/*
c++ `root-config --cflags --glibs` -lrooFit -lrooFitCore -o createSamples_02 createSamples_02.cpp
*/
...
// create pdfs  ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
  ...
  w.factory ("Gaussian::G2x(x, meanx2[15.], sigmax2[10.])") ;
  w.factory ("Gaussian::G2y(y, meany2[1.6], sigmay2[0.7])") ;
  w.factory ("PROD::bkg_3(G2x, G2y)") ;
...
// build the dataset      ---- ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
  ...
  RooDataSet * bkg_3 = w.pdf ("bkg_3")->generate (
      RooArgSet (*w.var("x"), *w.var("y")),
      10000
    ) ;
  ...
  TNtuple bkg_3NT ("bkg_3NT","bkg_3NT","x:y") ;
  ...
  for (int i = 0 ; i < 10000 ; ++i)
    {
      ...
      x = bkg_3->get (i)->getRealValue ("x") ;
      y = bkg_3->get (i)->getRealValue ("y") ;
      bkg_3NT.Fill (x,y) ;    
    }
