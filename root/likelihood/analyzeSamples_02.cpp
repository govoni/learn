/*
c++ `root-config --cflags --glibs` -lrooFit -lrooFitCore -o analyzeSamples_02 analyzeSamples_02.cpp
*/
...
{
  double num = 0. ;
  for (int iSpecies = 0 ; iSpecies < sigPdfs.at (0).size () ; ++iSpecies)
    {
      double add = 1. ;
      for (int iVar = 0 ; iVar < x.size () ; ++iVar)
        {
          add *= sigPdfs.at (iVar).at (iSpecies)->Eval (*x.at (iVar)) ;        
        }
  ...
  double den = 0. ;
  for (int iSpecies = 0 ; iSpecies < bkgPdfs.at (0).size () ; ++iSpecies)
    {
      double add = 1. ;
      for (int iVar = 0 ; iVar < x.size () ; ++iVar)
        {
          add *= bkgPdfs.at (iVar).at (iSpecies)->Eval (*x.at (iVar)) ;        
        }
...
// read the samples ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
  ...
  vector< vector<TF1*> > Pdfs ; // Pdfs[ivar][ispecies]
