...
//  is the owner of all geometry objects defined in a session,
//  therefore users must not try to control their deletion.
...
// defining displacements
   ...
   // translation
   TGeoTranslation *tr1 = new TGeoTranslation(0., 20., 0.);
   
   // roto-translation
   TGeoCombiTrans *combi1 = new TGeoCombiTrans(0.,-20.,0., 
                                   new TGeoRotation("rot1",0.,0.,180.));
...
// define the world volume
   ...
   Double_t worldy = 200.;
   Double_t worldz = 200.;
   TGeoVolume *top = geom->MakeBox("top","mat1",worldx,worldy,worldz);
   geom->SetTopVolume(top);
   
// define a beampipe along z axis

   Double_t pipemin = worldx / 20.;
   Double_t pipemax = worldx / 10.;
   TGeoVolume *pipe = geom->MakeTube("pipe","mat2",pipemin,pipemax,worldz-10);
   pipe->SetLineColor(30);

// putting it inside the world volume without any displacement

   top->AddNode(pipe,1,gGeoIdentity);

// creating an half cylindric detector
   
   Double_t decmin = worldx / 2;
   Double_t decmax = decmin * 1.5;
   Double_t declength = worldx / 2;
   Double_t decphistart = 0.;
   Double_t decphistop = 180.;
   TGeoVolume *halfdet =
		   geom->MakeTubs("halfdet","mat2",decmin,decmax,declength,decphistart,decphistop);
   halfdet->SetLineColor(50);

// putting two of them inside the world volume
   
   top->AddNode(halfdet,1,tr1);
   top->AddNode(halfdet,2,combi1);
...
//                -------------
   ...
   Double_t worldx = 110.;     // ma questi a che servono?
   Double_t worldy = 50.;
   Double_t worldz = 5.;
