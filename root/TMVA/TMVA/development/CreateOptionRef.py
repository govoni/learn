#!/usr/bin/env python
# @(#)root/tmva $Id: CreateOptionRef.py,v 1.24 2008/06/17 20:54:17 andreas.hoecker Exp $
# ------------------------------------------------------------------------------ #
...
class Configurable:
    ...
    def printOptions( self, type ):
        ...
        if type.lower() == 'html': 
            ...
            if self.type == 'classifier':
                text = 'Configuration options reference for classifier: <i>' + n.replace('Classifier::','') + '</i>'
            elif self.type == 'fitter':
                fitmethod = 'Unknown'
                if n.find('_GA') > 0:
            ...
            if self.type == 'classifier':
                ...
                print '<td align="right"><a STYLE="text-decoration: none" href="javascript:openWindow(\'%s\',\'%s\')" title="Tips how to tune the %s classifier"><img border=0 src="./images/i_icon.png" height="18px" width="18px" alt="Information on classifier tuning"/></a></td>' % (self.name, self.helptext, self.name.replace('Classifier::',''))
            else:
...
def main():
    ...
    for fname in filelist:
        ...
        for line in file:
            ...
            if line.find( 'Help for classifier' ) > 0:
                break;
        ...
        helptext += '<title>' + 'Classifier Info' + '</title>'
        ...
        helptext += '<th class="coltitle" style="font-family: sans-serif; font-size: 90%;">Tuning information for classifier: ' + configurable.getName().replace('Classifier::','') + '</th>'
        ...
        helptext += '</ul><hr><font color="#555555">Created on %s (TMVA, 2006 - 2008)</font>' % time.ctime()
    ...
    # first classifiers, then fitters, the other
    ctmp = []
    for c in configurables:                
        if c.type == 'classifier':
            ctmp.append( c )
    ...
    if outtype == 'html':
        ...
        print 'win = window.open("", name, "width=620,height=410,scrollbars=yes");'
        ...
        print '<tr><td><br>'
        print 'Reference for configuration options defined in the option string of a classifier booking, and for the definition of data sets used for training and testing (Factory).<br>'
        print '<p></p>'
        ...
        print '<td bgcolor=' + classifierSharedCol[0] + '>Options shared by all classifiers (through common base class).</td>'
        print '</tr><tr>'
        print '<td width=10></td><td width=130 bgcolor=' + classifierSpecificCol[0] + '><b>Bluish rows:</b></td>'
        print '<td bgcolor=' + classifierSpecificCol[0] + '>Specific classifier options.</td>'
        print '</tr><tr>'
        ...
        print '</tr></table>'
        print '<tr><td>'
        print '<br>Available classifiers/classes:<br>'
        print '<p></p>'
        ...
        print '<font color="#555555">Page created on %s (&copy; TMVA, 2006&minus;2008)</font></td>' % time.ctime()
