// @(#)root/tmva $Id: MethodFisher.cxx,v 1.12 2008/07/12 20:48:30 stelzer Exp $
...
Double_t TMVA::MethodFisher::GetMvaValue()
{
   ...
   for (UInt_t ivar=0; ivar<GetNvar(); ivar++)
      result += (*fFisherCoeff)[ivar]*ev->GetVal(ivar);
...
void TMVA::MethodFisher::GetMean( void )
{
   ...
   for (Int_t ievt=0; ievt<Data()->GetNEvents(); ievt++) {
      ...
      if (ev->IsSignal()) fSumOfWeightsS += weight;
      else                fSumOfWeightsB += weight;

      Double_t* sum = ev->IsSignal() ? sumS : sumB;
...
void TMVA::MethodFisher::PrintCoefficients( void ) 
{
   ...
   if (IsNormalised()) {
      ...
      Int_t maxL = 0;
      for (UInt_t ivar=0; ivar<GetNvar(); ivar++) if (GetInputExp(ivar).Length() > maxL) maxL = GetInputExp(ivar).Length();
