...
void TMVAGui( const char* fName = "TMVA.root" ) 
{
   ...
   cbar->AddButton( Form( "(%i) [ Boosting Process Plots ]", ++ic ),
                    Form( ".x boost.C", fName ),
                    "Plots histograms monitoring the error rate and boost weight of each boosted classifier",
                    buttonType );
