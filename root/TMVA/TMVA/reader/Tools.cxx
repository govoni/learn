// @(#)root/tmva $Id: Tools.cxx,v 1.8 2008/07/20 20:44:28 stelzer Exp $   
                               ...
                               Bool_t    norm )
{
   // sanity check
   if (0 == valVec) 
      Logger() << kFATAL << "<Tools::ComputeStat> value vector is zero pointer" << Endl;
   
   if ( events.size() != valVec->size() ) 
      Logger() << kFATAL << "<Tools::ComputeStat> event and value vector have different lengths " 
...
//_______________________________________________________________________
std::vector<TString>
TMVA::Tools::SplitString(const TString& theOpt, const char separator )
{
   ...
   while (splitOpt.Length()>0) {
      if ( ! splitOpt.Contains(separator) ) {
         ...
         splitOpt = splitOpt(splitOpt.First(separator)+1,splitOpt.Length());
      }
   }
   return splitV;
}
