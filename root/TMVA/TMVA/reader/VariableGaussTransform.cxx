// @(#)root/tmva $Id: VariableGaussTransform.cxx,v 1.2 2008/08/01 12:19:17 speckmayer Exp $
...
TMVA::VariableGaussTransform::VariableGaussTransform(const std::vector<VariableInfo>& varinfo )
   : VariableTransformBase( varinfo, Types::kGaussDecorr, "Gauss" ),
     fApplyGaussTransform(kFALSE),
     fApplyDecorrTransform(),
...
Bool_t TMVA::VariableGaussTransform::PrepareTransformation( const std::vector<Event*>& events )
{
   ...
   if (GetNVariables() > 200) { 
      fLogger << kINFO << "----------------------------------------------------------------------------" 
              << Endl;
      fLogger << kINFO 
              << ": More than 200 variables, will not calculate decorrelation matrix!" << Endl;
      fLogger << kINFO << "----------------------------------------------------------------------------" 
              << Endl;
      return kFALSE;
   }   
...
//_______________________________________________________________________
std::vector<TString>* TMVA::VariableGaussTransform::GetTransformationStrings( Types::ESBType type ) const
{
   ...
   for (Int_t ivar=0; ivar<nvar; ivar++) {
      ...
      for (Int_t jvar=0; jvar<nvar; jvar++) {
         ...
         str += Form( "%10.5g*%s", 
                      ...
                      (TString("[") + Variables()[jvar].GetExpression() + "]").Data() );
      }
...
//_______________________________________________________________________
const TMVA::Event* TMVA::VariableGaussTransform::Transform(const Event* const ev, Types::ESBType type ) const
{
...
void TMVA::VariableGaussTransform::GetCumulativeDist( const std::vector<Event*>& events )
{
   ...
   for (UInt_t iev=0; iev<nevt; iev++) {
      // fill the event
      const Event* ev = events[iev];
      
      for (UInt_t ivar=0; ivar<nvar; ivar++) {
         if (iev == 0) {
            xmin(ivar) = ev->GetVal(ivar);
            xmax(ivar) = ev->GetVal(ivar);
         }
         else {
            xmin(ivar) = TMath::Min( xmin(ivar), ev->GetVal(ivar) );
            xmax(ivar) = TMath::Max( xmax(ivar), ev->GetVal(ivar) );
         }
      }
   }
   ...
   for (UInt_t ivar=0; ivar<nvar; ivar++) {
      for (UInt_t i=0; i<2; i++) {
         ...
         fCumulativeDist[ivar][i] = new TH1F(Form("Cumulative_Var%d_%c",ivar,SB[i]),
                                             ...
                                             xmin(ivar),xmax(ivar));
         fCumulativeDist[ivar][i]->SetDirectory(0);
      }
   }
   ...
   for (UInt_t sb=0; sb<2; sb++) {
      ...
      //predefined types...
      Types::ESBType  type;
      type = (sb==0) ? Types::kSignal : Types::kBackground;
      ...
      for (UInt_t ievt=0; ievt<nevt; ievt++) {
         ...
         if (ev->IsSignal() == (type==Types::kSignal) ) {
            ic++; // count used events
            for (UInt_t ivar=0; ivar<nvar; ivar++) {
               tmpCumulator[ivar][sb]->Fill(ev->GetVal(ivar));;               
            }
         }
...
//_______________________________________________________________________
void TMVA::VariableGaussTransform::GetSQRMats( const std::vector<Event*>& events )
{
   ...
   for (UInt_t i=0; i<2; i++) {
      ...
      GetCovarianceMatrix( events, (i==Types::kSignal),  covMat );
...
//_______________________________________________________________________
void TMVA::VariableGaussTransform::GetCovarianceMatrix( const std::vector<Event*>& events, Bool_t isSignal, TMatrixDBase* mat )
{
   ...
   for (UInt_t ievt=0; ievt<nevt; ievt++) {
      ...
      if (ev->IsSignal() == isSignal) {
         ...
         for (ivar=0; ivar<nvar; ivar++) {
            ...
            Double_t xi = ( (IsNormalised()) ? gTools().NormVariable( ev->GetVal(ivar), xmin(ivar), xmax(ivar) )
                            : ev->GetVal(ivar) );
            vec(ivar) += xi;
            mat2(ivar, ivar) += (xi*xi);

            for (jvar=ivar+1; jvar<nvar; jvar++) {
               Double_t xj =  ( (IsNormalised()) ? gTools().NormVariable( ev->GetVal(jvar), xmin(ivar), xmax(ivar) )
                                : ev->GetVal(jvar) );
...
//_______________________________________________________________________
void TMVA::VariableGaussTransform::WriteTransformationToStream( std::ostream& o ) const
{
...
//_______________________________________________________________________
void TMVA::VariableGaussTransform::MakeFunction( std::ostream& fout, const TString& fcncName, Int_t part, UInt_t /*trCounter*/ ) 
{
