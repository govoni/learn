// @(#)root/tmva $Id: Timer.cxx,v 1.3 2008/06/22 18:21:13 stelzer Exp $   
...
{
   // constructor
   if (TString(prefix) == "") fPrefix = Timer::fgClassName;
   else              fPrefix = prefix;
...
void TMVA::Timer::DrawProgressBar( Int_t icounts ) 
{
   ...
   clog << fLogger->GetPrintedSource();
   if (fColourfulOutput) clog << gTools().Color("white_on_green") << gTools().Color("dyellow") << "[" << gTools().Color("reset");
   else                  clog << "[";
   ...
   // timing information
   if (fColourfulOutput) {
      clog << gTools().Color("reset") << " " ;
      clog << "(" << gTools().Color("red") << Int_t((100*(icounts+1))/Float_t(fNcounts)) << "%" << gTools().Color("reset")
               << ", " 
               << "time left: "
