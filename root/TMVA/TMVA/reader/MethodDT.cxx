// @(#)root/tmva $Id: MethodDT.cxx,v 1.6 2008/08/04 22:01:52 helgevoss Exp $ 
   ...
   : TMVA::MethodBase( jobName, methodTitle, theData, theOption, theTargetDir )
{
   ...
   // this one is supposed to be "boosted" using the "committee" boosting
   InitDT(); 
...
TMVA::MethodDT::MethodDT( DataSetInfo& dsi, 
                          ...
                          TDirectory* theTargetDir )
   : TMVA::MethodBase( dsi, theWeightFile, theTargetDir )
{
...
void TMVA::MethodDT::ProcessOptions() 
{
   ...
   if (fRandomisedTrees){
      fLogger << kINFO << " Randomised trees should use *bagging* as *boost* method. Did you set this in the *MethodBoost* ? . Here I can enforce only the *no pruning*" << Endl;
...
//_______________________________________________________________________
void TMVA::MethodDT::InitDT( void )
{
   // common initialisation with defaults for the DT-Method
   SetMethodName( "DT" );
   SetMethodType( Types::kDT );
   SetTestvarName();
...
void TMVA::MethodDT::Train( void )
{
   ...
   if (fRandomisedTrees) fLogger<<kWARNING<<" randomised Trees do not work yet in this framework," 
                                << " as I do not know how to give each tree a new random seed, now they"
                                << " will be all the same and that is not good " << Endl;

   fTree->BuildTree(Data()->GetEventCollection());
}
...
Bool_t TMVA::MethodDT::MonitorBoost( MethodBoost* booster )
{
       ...
       && !(fPruneMethod == DecisionTree::kNoPruning))
   {
      ...
      {
         fLogger << kINFO << "Pruning "<< booster->GetBoostNum() << " Decision Trees ... patience please" << Endl;
      }
...
//_______________________________________________________________________
Double_t TMVA::MethodDT::GetMvaValue()
{
