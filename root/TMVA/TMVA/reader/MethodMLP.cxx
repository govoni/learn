// @(#)root/tmva $Id: MethodMLP.cxx,v 1.11 2008/07/20 20:44:28 stelzer Exp $
...
TMVA::MethodMLP::MethodMLP( TString jobName, TString methodTitle, DataSetInfo& theData, 
   ...
   : MethodANNBase( jobName, methodTitle, theData, theOption, theTargetDir )
{
   ...
   if (fBPMode == kBatch) {
      Int_t numEvents = Data()->GetNTrainingEvents();
      if (fBatchSize < 1 || fBatchSize > numEvents) fBatchSize = numEvents;
   }
   std::cout << "Done MLP const" << std::endl;
}

//______________________________________________________________________________
TMVA::MethodMLP::MethodMLP( DataSetInfo& theData, TString theWeightFile, TDirectory* theTargetDir )
...
{
   // get the desired output of this event
   return GetEvent()->IsSignal()?fActivation->GetMax():fActivation->GetMin();
}
