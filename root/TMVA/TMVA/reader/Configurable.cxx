// @(#)root/tmva $Id: Configurable.cxx,v 1.6 2008/04/05 10:35:39 stelzer Exp $
...
//_______________________________________________________________________
void TMVA::Configurable::ParseOptions( Bool_t verbose, Bool_t readarrays ) 
{
   // options parser
   if (verbose) {
   ...
   while (TObjString * os = (TObjString*) setOptIt()) { // loop over parsed options
      ...
      if (s.Contains('=')) { // desired way of setting an option: "...:optname=optvalue:..."
         ...
         if (optname.Contains('[') && readarrays) {
