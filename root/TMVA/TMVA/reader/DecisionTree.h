// @(#)root/tmva $Id: DecisionTree.h,v 1.6 2008/07/23 16:49:04 helgevoss Exp $
...
namespace TMVA {
   ...
   public:
      ...
      virtual Node * CreateNode(UInt_t) { return new DecisionTreeNode(); }
  
      // building of a tree by recursivly splitting the nodes 
      Int_t BuildTree( const std::vector<TMVA::Event*> & eventSample, 
                       DecisionTreeNode *node = NULL );

      // determine the way how a node is split (which variable, which cut value)
      Double_t TrainNode( const std::vector<TMVA::Event*> & eventSample,  DecisionTreeNode *node ) { return TrainNodeFast( eventSample, node ); }
      Double_t TrainNodeFast( const std::vector<TMVA::Event*> & eventSample,  DecisionTreeNode *node );
      Double_t TrainNodeFull( const std::vector<TMVA::Event*> & eventSample,  DecisionTreeNode *node );

      // fill at tree with a given structure already (just see how many signa/bkgr
      // events end up in each node 
      void FillTree( std::vector<TMVA::Event*> & eventSample);
   ...
   private:
      ...
      void SetCutPoints(std::vector<Double_t> & cut_points,
                        Double_t xmin,
                        Double_t xmax,
      ...
      Double_t SamplePurity(std::vector<Event*> eventSample);
