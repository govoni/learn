...
Bool_t Use_BoostedLikelihood = 0;
...
// ---
Bool_t Use_BDT               = 1;
Bool_t Use_BoostedDT         = 1;
Bool_t Use_BoostedDTAltDef   = 1; //alternative way to define boosting
Bool_t Use_BDTD              = 0;
// ---
...
void TMVAnalysis( TString myMethodList = "" ) 
{
   ...
   if (mlist->GetSize()>0) {
      Use_CutsGA = Use_CutsD = Use_Cuts
         = Use_LikelihoodKDE = Use_LikelihoodMIX = Use_LikelihoodPCA = Use_LikelihoodD 
         = Use_BoostedLikelihood = Use_Likelihood
         = Use_PDERSPCA = Use_PDERSD = Use_PDERS = Use_PDERSkNN
         = Use_KNN
         = Use_MLP = Use_CFMlpANN = Use_TMlpANN
         = Use_HMatrix = Use_BoostedFisher = Use_Fisher = Use_BDTD = Use_BDT
         = Use_BoostedDT = Use_BoostedDTAltDef= Use_RuleFit = Use_RuleFitJF
      ...
      if (mlist->FindObject( "BoostedLikelihood" ) != 0) Use_BoostedLikelihood = 1; 
      ...
      if (mlist->FindObject( "BoostedFisher"     ) != 0) Use_BoostedFisher     = 1; 
      ...
      if (mlist->FindObject( "BoostedDT"         ) != 0) Use_BoostedDT         = 1; 
      if (mlist->FindObject( "BoostedDTAltDef"   ) != 0) Use_BoostedDTAltDef   = 1; 
   ...
   if (Use_BoostedLikelihood) 
   {
      factory->BookMethod( TMVA::Types::kLikelihood, "BoostedLikelihood", 
                           "!H:!V:!TransformOutput:PDFInterpol=Spline2:NSmoothSig[0]=10:NSmoothBkg[0]=10:NSmoothBkg[1]=10:NSmooth=10:NAvEvtPerBin=50:BoostNum=100:BoostType=AdaBoost:!RecalculateMVACut" );
   }
   ...
   if (Use_Fisher)
      factory->BookMethod( TMVA::Types::kFisher, "Fisher", 
                           "H:!V:!Normalise:CreateMVAPdfs:Fisher:NbinsMVAPdf=50:NsmoothMVAPdf=6:MinNsmoothMVAPdfBkg=1:MaxNsmoothMVAPdfBkg=50" );    
   if (Use_BoostedFisher)
      factory->BookMethod( TMVA::Types::kFisher, "BoostedFisher", 
                           "H:!V:!Normalise:CreateMVAPdfs:Fisher:NbinsMVAPdf=50:NsmoothMVAPdf=1:BoostNum=200:BoostType=AdaBoost" );
   ...
   if (Use_BoostedDT)
      factory->BookMethod( TMVA::Types::kDT, "BoostedDT", 
                           "!H:!V:BoostNum=200:BoostType=AdaBoost:SeparationType=GiniIndex:nCuts=20:PruneMethod=CostComplexity:PruneStrength=4.5:VarTransform=Decorrelate" );
   if (Use_BoostedDTAltDef)
   {
      factory->BookMethod( TMVA::Types::kBoost, "BoostedDT",
                           "BoostNum=200:BoostType=AdaBoost");
      ((TMVA::MethodBoost*)factory->GetMethod("BoostedDT"))->BookMethod( TMVA::Types::kDT,"DT", 
                           "!H:!V:SeparationType=GiniIndex:nCuts=20:PruneMethod=CostComplexity:PruneStrength=4.5:VarTransform=Decorrelate" );
   }
