// @(#)root/tmva $Id: PDF.h,v 1.6 2008/06/22 18:21:13 stelzer Exp $
...
namespace TMVA {
   ...
   public:
      ...
      PDF();
      PDF ( const TH1* theHist, EInterpolateMethod method = kSpline2, 
      Int_t minnsmooth = 0, Int_t maxnsmooth = 0, Bool_t checkHist = kFALSE );
      ...
      PDF (const TString& options, const TString& suffix = "", PDF* defaultPDF = 0);
      virtual ~PDF();
      ...
      //returns the processed option string
      //TString  GetOptions()      { return fConfig->GetOptions();}
   ...
   private:
      ...
      //reads from and option string the definitions for pdf with suffix and returns it
      void     ReadOptionString( const TString& suffix = "", PDF* defaultPDF = 0);
      ...
      //Configurable*   fConfig;                  //contains the PDF's configurations

      Int_t    fNsmooth;                        // Min number of smoothing iterations, for bins with most data
      Int_t    fMinNsmooth;                           // Min number of smoothing iterations, for bins with most data
      Int_t    fMaxNsmooth;                           // Max number of smoothing iterations, for bins with least data
      ...
      TString  fKDEtypeString;                        //string used to read definitions
