// @(#)root/tmva $Id: MethodFDA.cxx,v 1.11 2008/08/06 14:28:43 helgevoss Exp $    
...
void TMVA::MethodFDA::Train( void )
{
   ...
   for (Int_t ievt=0; ievt<Data()->GetNEvents(); ievt++) {
      ...
      // true event copy
      //      Event*  ev = new Event( *GetEvent() ); // not needed any longer - Joerg
...
//      sigBack = ev->GetValue( dimension );
      result    = InterpretFormula( ev, pars );
      deviation = (result - desired )*(result-desired);

      Int_t itype = Int_t(desired);
//      std::cout << "ievt " << ievt << "  itype " << itype << "  desired " << desired << "  result " << result << "  deviation " << deviation << " weight " << ev->GetWeight() << "  w-sum " << sumOfWeights[itype] << " est[0] " << estimator[0] << " est[1] " << estimator[1] << std::endl;
      estimator[itype] += deviation * ev->GetWeight();
   }
