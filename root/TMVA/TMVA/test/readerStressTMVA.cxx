// $Id: readerStressTMVA.cxx,v 1.5 2008/08/06 21:04:25 speckmayer Exp $
...
class TMVATest {
   ...
   public:
      ...
      bool test_event();
      bool test_createDataSetFromTree();
      bool test_pdf_streams();
      bool test_factory_basic();
      bool test_reader_basic();
      bool test_variable_transforms();
...
//      std::cout << "iv : "<< iv  << " val: " << ptrEv->GetVal( iv ) << " evval: " << ev[iv] << std::endl;
      assert( fabs( ptrEv->GetVal(iv) - ev[iv] ) < 1e-10 );
   }
   delete ptrEv;
   ...
   assert( fabs( var1 - evt2->GetVal( 0 ) ) < 1e-10 );
   delete evt2;
...
bool TMVATest::test_createDataSetFromTree(){
...
bool TMVATest::test_pdf_streams(){
...
bool TMVATest::test_variable_transforms(){
...
//   TMVA::Event::ClearDynamicVariables();
   ...
   vars.push_back( VariableInfo( "var1", 0, 'F' ) );
   vars.push_back( VariableInfo( "var2", 0, 'F' ) );
   vars.push_back( VariableInfo( "var3", 0, 'F' ) );
   vars.push_back( VariableInfo( "var4", 0, 'F' ) );

   // create some transforms
   std::vector<VariableTransformBase*> vtb;
   vtb.push_back( new TMVA::VariableGaussTransform( vars ) );
   vtb.push_back( new TMVA::VariableIdentityTransform( vars ) ); 
   vtb.push_back( new TMVA::VariableDecorrTransform( vars ) );
   vtb.push_back( new TMVA::VariablePCATransform( vars ) );

   // create some doubles for the transforms of which the values will be set 
   // from an input stream.
   std::vector<VariableTransformBase*> vtbRec;
   vtbRec.push_back( new TMVA::VariableGaussTransform( vars ) );
   vtbRec.push_back( new TMVA::VariableIdentityTransform( vars ) );
   vtbRec.push_back( new TMVA::VariableDecorrTransform( vars ) );
   vtbRec.push_back( new TMVA::VariablePCATransform( vars ) );
...
//      ptrEv->Print( std::cout );
      for( UInt_t iv = 0; iv < ptrEv->GetNVars(); iv++ ){
//	 std::cout << "iv : "<< iv  << " val: " << ptrEv->GetVal( iv ) << " evval: " << ev[iv] << std::endl;
	 assert( fabs( ptrEv->GetVal( iv ) - ev[iv] ) < 1e-10 ); // difference between original value and value stored in event
      }
   }
   ...
   for( std::vector<VariableTransformBase*>::iterator it = vtb.begin(); it < vtb.end(); it++ ){
	 ...
	 const Event* evOriginal = (*it)->Transform( (*itEv) );
	 const Event* evRecieved = (*itRec)->Transform( (*itEv) );
	 for( UInt_t iv = 0; iv < (*itEv)->GetNVars(); iv++ ){
	    bool isOK = fabs( evOriginal->GetVal( iv )-evRecieved->GetVal( iv ) )<1e-10;
...
bool TMVATest::test_factory_basic(){
	return false;
}
...
bool TMVATest::test_reader_basic(){
