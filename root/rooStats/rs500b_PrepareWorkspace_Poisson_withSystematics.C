     ...
     int type = 0 )
{
  ...
  myWS.factory("B[1,0,20]"); // value fixed to 1 and range [0,20]
  ...
  RooAbsData* data = 0;
  // type = 0 : binned data with fixed number of events
  if (type==0) data = myWS.pdf("model")->generateBinned(*myWS.set("observables"),myWS.var("S")->getVal()+myWS.var("B")->getVal(),Name("data"));
  // type = 1 : binned data with total number of events fluctuating using Poisson probability
