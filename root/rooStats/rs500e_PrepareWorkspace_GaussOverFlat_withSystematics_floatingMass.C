//////////////////////////////////////////////////////////////////////////
//
// RooStats tutorial macro #500
// Kyle Cranmer, modified from...
...
void rs500e_PrepareWorkspace_GaussOverFlat_withSystematics_floatingMass ( 
    TString fileName = "WS_GaussOverFlat_withSystematics_floatingMass.root", 
    int type = 1 
  )
{
  ...
  myWS.defineSet ("POI","S,trueMass") ;
  
  // Generate data
  RooAbsData* data = 0 ;
  // unbinned data with Poisson fluctuations for total number of events
  if (type == 0) data = myWS.pdf ("model")->generate (
      *myWS.set ("observables"),
//      NumEvents (10000), // circa, di default e' il numero di eventi impostati
                           // nel modello
      Extended (),
      Name ("data")
    ) ;  
  // binned data with Poisson fluctuations for total number of events
  if (type == 1) data = myWS.pdf ("model")->generateBinned (
      *myWS.set ("observables"),
      Extended (),
      Name ("data")
    ) ;  
  // binned without any fluctuations (average case)
  if (type == 2) data = myWS.pdf ("model")->generateBinned (
      *myWS.set ("observables"),
      Name ("data"),
      ExpectedData ()
    ) ;
