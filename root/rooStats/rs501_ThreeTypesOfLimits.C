...
void rs501_ThreeTypesOfLimits ( const char* fileName="WS_GaussOverFlat_withSystematics_floatingMass.root" )
{
  ...
  if ( priorNuisance!=0 ) model = new RooProdPdf ("constrainedModel",
  ...
  // the 95% CL one-sided upper-limit is the same as the two-sided upper-limit of a 90% CL interval  
  ...
  fc.UseAdaptiveSampling (true) ; //PG questo e' un cambio di coordinate?
  ...
  mc.SetNumIters (1000000) ; // steps in the chain
  ...
  std::cout << "PIETRO " << fcint << std::endl ;
  if (fcint)
